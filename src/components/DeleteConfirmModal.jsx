...
// ─── Modal Component ───────────────────────────────────────────────
export function DeleteConfirmModal({ isOpen, driverName, onConfirm, onCancel }) {
  // Close on Escape key
  useEffect(() => {
      ...
      <div className="dcm-overlay" onClick={onCancel}>
        <div className="dcm-card" onClick={e => e.stopPropagation()}>
          ...
          <p className="dcm-body">
            Vous êtes sur le point de supprimer le camion assigné à <span className="dcm-driver">{driverName}</span>.
            Cette action est irréversible.
          </p>
 
          <div className="dcm-warning">
            ⚠️ Toutes les données liées à ce camion seront définitivement supprimées.
          </div>
